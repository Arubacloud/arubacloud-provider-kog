...
func PostKeypair(opts handlers.HandlerOptions) handlers.Handler {
	return &postHandler{baseHandler: newBaseHandler(opts)}
}

func PutKeypair(opts handlers.HandlerOptions) handlers.Handler {
	return &putHandler{baseHandler: newBaseHandler(opts)}
}

func ListKeypairs(opts handlers.HandlerOptions) handlers.Handler {
...
func (h *getHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	h.Log.Printf("Getting keypair %s for project %s", id, projectId)

	// Call Aruba Cloud SDK to get keypair
	response, err := client.FromCompute().KeyPairs().Get(r.Context(), projectId, id, params)
	if err != nil {
		h.Log.Printf("Failed to get keypair: %v", err)
...
func (h *postHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	h.Log.Printf("Creating keypair for project %s", projectId)

	// Call Aruba Cloud SDK to create keypair
	response, err := client.FromCompute().KeyPairs().Create(r.Context(), projectId, req, params)
	if err != nil {
		h.Log.Printf("Failed to create keypair: %v", err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h *putHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	// KeyPairs resource does not support Update operation
	h.Log.Print("PUT keypair operation not supported by Aruba Cloud API")

	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusMethodNotAllowed)
	json.NewEncoder(w).Encode(map[string]string{
		"message": "PUT operation not supported for keypairs",
	})
}
...
func (h *listHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	h.Log.Printf("Listing keypairs for project %s", projectId)

	// Call Aruba Cloud SDK to list keypairs
	response, err := client.FromCompute().KeyPairs().List(r.Context(), projectId, params)
	if err != nil {
		h.Log.Printf("Failed to list keypairs: %v", err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
