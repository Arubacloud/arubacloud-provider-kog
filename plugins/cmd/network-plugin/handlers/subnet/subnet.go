...
func (h *getHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	h.Log.Printf("Getting subnet %s for vpc %s in project %s", id, vpcId, projectId)

	// Call Aruba Cloud SDK to get subnet
	response, err := client.FromNetwork().Subnets().Get(r.Context(), projectId, vpcId, id, params)
	if err != nil {
		h.Log.Printf("Failed to get subnet: %v", err)
...
func (h *postHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	h.Log.Printf("Creating subnet for vpc %s in project %s", vpcId, projectId)

	// Call Aruba Cloud SDK to create subnet
	response, err := client.FromNetwork().Subnets().Create(r.Context(), projectId, vpcId, req, params)
	if err != nil {
		h.Log.Printf("Failed to create subnet: %v", err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h *putHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	h.Log.Printf("Updating subnet %s for vpc %s in project %s", id, vpcId, projectId)

	// Call Aruba Cloud SDK to update subnet
	response, err := client.FromNetwork().Subnets().Update(r.Context(), projectId, vpcId, id, req, params)
	if err != nil {
		h.Log.Printf("Failed to update subnet: %v", err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h *listHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	h.Log.Printf("Listing subnets for vpc %s in project %s", vpcId, projectId)

	// Call Aruba Cloud SDK to list subnets
	response, err := client.FromNetwork().Subnets().List(r.Context(), projectId, vpcId, params)
	if err != nil {
		h.Log.Printf("Failed to list subnets: %v", err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
